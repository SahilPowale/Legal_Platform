...
const Home = () => {
    ...
    // 1. ADMIN VIEW (🚨 NEW FIX 🚨)
    if (user && user.role === 'admin') {
        return (
            <div style={styles.container} className="fade-in">
                <div className="slide-up">
                    <span style={{ backgroundColor: '#fee2e2', color: '#dc2626', padding: '5px 15px', borderRadius: '20px', fontWeight: 'bold', fontSize: '0.9rem' }}>Admin Command Center</span>
                    <h1 style={{ ...styles.heroTitle, marginTop: '20px' }}>Welcome, Administrator</h1>
                    <p style={styles.subText}>Oversee platform verifications, manage users, and maintain platform integrity.</p>
                    <div style={styles.buttonGroup}>
                        <Link to="/admin" className="btn btn-primary" style={{ textDecoration: 'none' }}>Go to Admin Panel</Link>
                    </div>
                </div>
            </div>
        );
